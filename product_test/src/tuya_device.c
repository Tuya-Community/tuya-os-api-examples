...
{
    PR_NOTICE("pt: mf_user_pre_gpio_test_cb");
    return;
}
...
{
    PR_NOTICE("pt: mf_user_enter_callback");
    return;
}
...
{
    PR_NOTICE("pt: mf_user_product_test_cb");
    return OPRT_OK;
}
...
/**
* @brief product test callback
*
* @param[in] flag: authorization flag 
* @param[in] rssi: signal strength 
* @return none
*/
static void app_prod_test_cb(BOOL_T flag, SCHAR_T rssi)
{
    PR_NOTICE("flag:%d, rssi:%d", flag, rssi);
    if (flag) {
        if (rssi < -60) {
            PR_WARN("weak network");
            /* do something */
        } else {
            PR_NOTICE("product test begin");
            /* do something */
            PR_NOTICE("product test end");
        }
    } else {
        PR_ERR("no uuid and auth_key");
    }
    return;
}
...
VOID app_init(VOID) 
{
    ...
    wifi_config_init();
    app_cfg_set(WIFI_WORK_MODE_SEL, app_prod_test_cb);
...
OPERATE_RET device_init(VOID) 
{
    ...
    op_ret = tuya_iot_wf_soc_dev_init_param(WIFI_WORK_MODE_SEL, WF_START_AP_ONLY, &wf_cbs, NULL, PRODECT_ID, DEV_SW_VERSION);
    if(OPRT_OK != op_ret) {
